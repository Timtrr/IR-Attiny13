...
uint32_t readNEC() {
  ...
  while (digitalRead(IR_PIN) == HIGH)
    ;                       // Ждем, пока сигнал не станет низким
...
// Функция для сохранения 32-битного значения в EEPROM
void saveToEEPROM(uint16_t address, uint32_t value) {
  for (uint8_t i = 0; i < 4; i++) {
    EEPROM.write(address + i, (value >> (i * 8)) & 0xFF);  // Сохраняем байт за байтом
  }
}

// Функция для чтения 32-битного значения из EEPROM
uint32_t readFromEEPROM(uint16_t address) {
  uint32_t value = 0;
  for (uint8_t i = 0; i < 4; i++) {
    value |= (uint32_t(EEPROM.read(address + i)) << (i * 8));  // Читаем байт за байтом
  }
  return value;
}
...
void setup() {
  ...
  // Читаем сохранённую команду из EEPROM при старте
  savedCommand = readFromEEPROM(0);
...
void loop() {
  ...
  if (digitalRead(DEBUG_PIN) == LOW) {  // Проверяем состояние кнопки (кнопка замкнута на землю)
    ...
    if (command != 0) {
      savedCommand = command;         // Сохраняем команду в переменную
      saveToEEPROM(0, savedCommand);  // Сохраняем команду в EEPROM
      blinkLED(5);                    // Мигаем светодиодом 3 раза, чтобы показать, что команда сохранена
    }
